...
from app.assistant.prompts import SYSTEM_PROMPT, USER_NO_CONTEXT, USER_WITH_CONTEXT
...
async def ask(question: str, history: list[dict[str, str]] | None = None) -> dict:
    ...
    # Формируем пользовательский prompt. Роль НЕ дублируем — она уходит один раз
    # в system-сообщении (SYSTEM_PROMPT). При пустом контексте (например все
    # чанки отсеклись порогом релевантности) используем шаблон без контекста.
    if context:
        prompt = USER_WITH_CONTEXT.format(context=context, question=question)
    else:
        prompt = USER_NO_CONTEXT.format(question=question)

    # Формируем messages для LLM
    messages: list[dict[str, str]] = []

    # System prompt с ролью — единственный источник роли (важно для OpenRouter и Gemini)
    ...
    if history:
        for msg in history[-10:]:
            ...
            messages.append({"role": llm_role, "content": msg.get("text") or ""})
